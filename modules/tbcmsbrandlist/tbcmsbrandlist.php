...
include_once('classes/tbcmsbrandlist_status.class.php');
include_once('classes/tbcmsbrandlist_image_upload.class.php');

class TbcmsBrandList extends Module
{
    public $id_shop_group = '';
    public $id_shop = '';
    public function __construct()
    {
        $this->name = 'tbcmsbrandlist';
        ...
        $this->displayName = $this->l('TemplateBeta - Brand List');
    ...
    public function createVariable()
    {
        ...
        foreach ($languages as $lang) {
            $result['TBCMSBRANDLIST_TITLE'][$lang['id_lang']] = 'Our Brands';
            $result['TBCMSBRANDLIST_SUB_DESCRIPTION'][$lang['id_lang']] = 'Sub Description';
            $result['TBCMSBRANDLIST_DESCRIPTION'][$lang['id_lang']] = 'Description';
            $result['TBCMSBRANDLIST_IMG'][$lang['id_lang']] = 'demo_main_img.jpg';
        }

        Configuration::updateValue('TBCMSBRANDLIST_TITLE', $result['TBCMSBRANDLIST_TITLE']);
        Configuration::updateValue('TBCMSBRANDLIST_SUB_DESCRIPTION', $result['TBCMSBRANDLIST_SUB_DESCRIPTION']);
        Configuration::updateValue('TBCMSBRANDLIST_DESCRIPTION', $result['TBCMSBRANDLIST_DESCRIPTION']);
        Configuration::updateValue('TBCMSBRANDLIST_IMG', $result['TBCMSBRANDLIST_IMG']);
    }
    ...
    {
        $create_table = array();
        $create_table[] = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tbcmsbrandlist` (
                        `id_tbcmsbrandlist` int(11) AUTO_INCREMENT PRIMARY KEY,
                        `position` int(11),
        ...
        $create_table[] = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tbcmsbrandlist_lang` (
                        `id_tbcmsbrandlist_lang` INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        `id_tbcmsbrandlist` INT NOT NULL,
                        `id_shop_group` int(11),
                        `id_shop` int(11),
    ...
    public function insertSmapleData($num_of_data)
    {
        ...
        for ($i = 1; $i<=$num_of_data; $i++) {
            ...
            foreach ($languages as $lang) {
                $demo_data[] = 'INSERT INTO
                                `'._DB_PREFIX_.'tbcmsbrandlist_lang`
                            SET
                                `id_tbcmsbrandlist_lang` = NULL,
                                `id_tbcmsbrandlist` = '.$i.',
                                `id_shop_group` = '.$this->id_shop_group.',
                                `id_shop` = '.$this->id_shop.',
    ...
    {
        $select_data = 'SELECT MAX(id_tbcmsbrandlist) as max_id FROM `'._DB_PREFIX_.'tbcmsbrandlist`';
        $ans = Db::getInstance()->executeS($select_data);
        return $ans[0]['max_id'];
    }

    public function selectAllLangIdById($id_tbcmsbrandlist)
    {
        $select_data = 'SELECT 
                            id_lang 
                        FROM 
                            `'._DB_PREFIX_.'tbcmsbrandlist_lang` 
                        WHERE 
                            id_tbcmsbrandlist = '.$id_tbcmsbrandlist;
        $ans = Db::getInstance()->executeS($select_data);
        $return = array();
        foreach ($ans as $a) {
            $return[] = $a['id_lang'];
    ...
    public function showAdminData()
    {
        ...
        $select_data = 'SELECT * FROM `'._DB_PREFIX_.'tbcmsbrandlist`'
            .' WHERE `id_shop_group` = '.$this->id_shop_group.' AND `id_shop` = '.$this->id_shop
            .' ORDER BY `position`;';

        $result['tbcmsbrandlist'] = Db::getInstance()->executeS($select_data);

        $select_data = 'SELECT * FROM `'._DB_PREFIX_.'tbcmsbrandlist_lang`'
            .' WHERE `id_shop_group` = '.$this->id_shop_group.' AND `id_shop` = '.$this->id_shop.';';


        $result['tbcmsbrandlist_lang'] = Db::getInstance()->executeS($select_data);

        foreach ($result['tbcmsbrandlist'] as $key => $data) {
            $return_data[$key]['id'] = $data['id_tbcmsbrandlist'];
            $id = $data['id_tbcmsbrandlist'];

            foreach ($result['tbcmsbrandlist_lang'] as $lang) {
                if ($default_lang_id == $lang['id_lang'] && $id == $lang['id_tbcmsbrandlist']) {
    ...
    public function showData($id = null)
    {
        ...
        $select_data .= 'SELECT * FROM `'._DB_PREFIX_.'tbcmsbrandlist` 
                ...
                .' AND `id_shop` = '.$this->id_shop;
        if ($id) {
            $select_data .= ' AND `id_tbcmsbrandlist` = '.$id;
        } else {
    ...
    public function showFrontData()
    {
        ...
        $select_data = '
            SELECT 
                mainTable.id_tbcmsbrandlist AS code,
                mainTable.image,
                mainTable.link,
                subTable.title,
            ...
            FROM 
                `'._DB_PREFIX_.'tbcmsbrandlist` mainTable
            LEFT JOIN
                '._DB_PREFIX_.'tbcmsbrandlist_lang subTable
            ON
                mainTable.id_tbcmsbrandlist = subTable.id_tbcmsbrandlist
            WHERE 
    ...
    {
        Configuration::deleteByName('TBCMSBRANDLIST_TITLE');
        Configuration::deleteByName('TBCMSBRANDLIST_SUB_DESCRIPTION');
        Configuration::deleteByName('TBCMSBRANDLIST_DESCRIPTION');
        Configuration::deleteByName('TBCMSBRANDLIST_IMG');
    }
    ...
    public function deleteRecord($id)
    {
        ...
        $delete_data = array();
        $delete_data[] = 'DELETE FROM `'._DB_PREFIX_.'tbcmsbrandlist`
            WHERE 
                    `id_shop_group` = '.$this->id_shop_group.'
                AND 
                    `id_shop` = '.$this->id_shop.' 
        ...
        $delete_data[] = 'DELETE FROM `'._DB_PREFIX_.'tbcmsbrandlist_lang` 
            WHERE 
                    `id_shop_group` = '.$this->id_shop_group.'
                AND 
                    `id_shop` = '.$this->id_shop.' 
    ...
    {
        $delete_table = array();
        $delete_table[] = 'DROP TABLE IF EXISTS `'._DB_PREFIX_.'tbcmsbrandlist`';
        $delete_table[] = 'DROP TABLE IF EXISTS `'._DB_PREFIX_.'tbcmsbrandlist_lang`';
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submittbcmsBrandListForm')) {
            $old_file = '';
            $result['id'] = '';
            ...
            if (!$no_image_selected) {
                ...
                $this->clearCustomSmartyCache('tbcmsbrandlist_display_home.tpl');
        ...
        if (Tools::isSubmit('submittbcmsBrandListMainTitleForm')) {
            $languages = Language::getLanguages();
            $obj_image = new TbcmsBrandListImageUpload();
            foreach ($languages as $lang) {
                if (!empty($_FILES['TBCMSBRANDLIST_IMG_'.$lang['id_lang']]['name'])) {
                    $old_file = Configuration::get('TBCMSBRANDLIST_IMG', $lang['id_lang']);
                    $new_file = $_FILES['TBCMSBRANDLIST_IMG_'.$lang['id_lang']];
                    $ans = $obj_image->imageUploading($new_file, $old_file);
                    if ($ans['success']) {
                        $result['TBCMSBRANDLIST_IMG'][$lang['id_lang']] = $ans['name'];
                    } else {
                        $message .= $ans['error'];
                        $result['TBCMSBRANDLIST_IMG'][$lang['id_lang']] = $old_file;
                    }
                } else {
                    $old_file = Configuration::get('TBCMSBRANDLIST_IMG', $lang['id_lang']);
                    $result['TBCMSBRANDLIST_IMG'][$lang['id_lang']] = $old_file;
                ...
                $tmp = Tools::getValue('TBCMSBRANDLIST_TITLE_'.$lang['id_lang']);
                $result['TBCMSBRANDLIST_TITLE'][$lang['id_lang']] = $tmp;

                $tmp = Tools::getValue('TBCMSBRANDLIST_SUB_DESCRIPTION_'.$lang['id_lang']);
                $result['TBCMSBRANDLIST_SUB_DESCRIPTION'][$lang['id_lang']] = $tmp;

                $tmp = Tools::getValue('TBCMSBRANDLIST_DESCRIPTION_'.$lang['id_lang']);
                $result['TBCMSBRANDLIST_DESCRIPTION'][$lang['id_lang']] = $tmp;
            }
            Configuration::updateValue('TBCMSBRANDLIST_IMG', $result['TBCMSBRANDLIST_IMG']);
            Configuration::updateValue('TBCMSBRANDLIST_TITLE', $result['TBCMSBRANDLIST_TITLE']);
            $tmp = $result['TBCMSBRANDLIST_SUB_DESCRIPTION'];
            Configuration::updateValue('TBCMSBRANDLIST_SUB_DESCRIPTION', $tmp);

            $tmp = $result['TBCMSBRANDLIST_DESCRIPTION'];
            Configuration::updateValue('TBCMSBRANDLIST_DESCRIPTION', $tmp);
    ...
    protected function renderForm()
    {
        ...
        if ($show_fields['main_status']) {
            $form[] = $this->tbcmsBrandListMainTitleForm();
        }

        if ($show_fields['record_form']) {
            $form[] = $this->tbcmsBrandListForm();
        }
    ...
    protected function tbcmsBrandListMainTitleForm()
    {
        $tbcms_obj = new TbcmsBrandListStatus();
        $show_fields = $tbcms_obj->fieldStatusInformation();
        $input = array();
        ...
        if ($show_fields['main_sub_title']) {
            $input[] = array(
                    'col' => 7,
                    'type' => 'text',
                    'name' => 'TBCMSBRANDLIST_SUB_DESCRIPTION',
        ...
        if ($show_fields['main_description']) {
            $input[] = array(
                    'col' => 7,
                    'type' => 'text',
                    'name' => 'TBCMSBRANDLIST_DESCRIPTION',
        ...
        return array(
            'form' => array(
                ...
                'submit' => array(
                    'title' => $this->l('Save'),
                    'name' => 'submittbcmsBrandListMainTitleForm',
                ),
            ),
        );
    }
    ...
    protected function tbcmsBrandListForm()
    {
        $tbcms_obj = new TbcmsBrandListStatus();
        $show_fields = $tbcms_obj->fieldStatusInformation();
        $input = array();
    ...
    protected function getConfigFormValues()
    {
        ...
        foreach ($languages as $lang) {
            $tmp = Configuration::get('TBCMSBRANDLIST_TITLE', $lang['id_lang']);
            $fields['TBCMSBRANDLIST_TITLE'][$lang['id_lang']] = $tmp;

            $tmp = Configuration::get('TBCMSBRANDLIST_SUB_DESCRIPTION', $lang['id_lang']);
            $fields['TBCMSBRANDLIST_SUB_DESCRIPTION'][$lang['id_lang']] = $tmp;

            $tmp = Configuration::get('TBCMSBRANDLIST_DESCRIPTION', $lang['id_lang']);
            $fields['TBCMSBRANDLIST_DESCRIPTION'][$lang['id_lang']] = $tmp;

            $tmp = Configuration::get('TBCMSBRANDLIST_IMG', $lang['id_lang']);
            $fields['TBCMSBRANDLIST_IMG'][$lang['id_lang']] = $tmp;
        }
    ...
    public function showAdminResult()
    {
        ...
        return $this->display(__FILE__, "views/templates/admin/tbcmsbrandlist_manage.tpl");
    }
    ...
    public function showFrontSideResult()
    {
        ...
        if ($main_heading['main_status']) {
            ...
            $main_heading_data['title'] = Configuration::get('TBCMSBRANDLIST_TITLE', $id_lang);
            $main_heading_data['short_desc'] = Configuration::get('TBCMSBRANDLIST_SUB_DESCRIPTION', $id_lang);
            $main_heading_data['desc'] = Configuration::get('TBCMSBRANDLIST_DESCRIPTION', $id_lang);
            $main_heading_data['image'] = Configuration::get('TBCMSBRANDLIST_IMG', $id_lang);
    ...
    {
        if (!Cache::isStored('tbcmsbrandlist_display_home.tpl')) {
            ...
            Cache::store('tbcmsbrandlist_display_home.tpl', $output);
        }

        return Cache::retrieve('tbcmsbrandlist_display_home.tpl');
    }
}
