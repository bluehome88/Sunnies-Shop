            ...
            {foreach $fields_display AS $key => $params}
                ...
                {/if}
                    ...
                    {elseif isset($params.activeVisu)}
                        {if $tr.$key}
                            <i class="icon-check-ok"></i>
                                {l s='Enabled' mod='tbcmsproductcomments'}
                        {else}
                            <i class="icon-remove"></i>
                            {l s='Disabled' mod='tbcmsproductcomments'}
                        {/if}
                    ...
                    {elseif isset($params.type) && $params.type == 'percent'}
                        {$tr.$key} {l s='%' mod='tbcmsproductcomments'}
                    {elseif isset($params.type) && $params.type == 'bool'}
                        {if $tr.$key == 1}
                            {l s='Yes' mod='tbcmsproductcomments'}
                        {elseif $tr.$key == 0 && $tr.$key != ''}
                            {l s='No' mod='tbcmsproductcomments'}
                        {/if}
            ...
            {if $has_actions}
                <td class="text-right">
                    ...
                    {if !empty(Module::getInstanceByName('reviewsreply')) && isset(Module::getInstanceByName('reviewsreply')->active)}
                        {if Module::getInstanceByName('reviewsreply')->active == true && $identifier == 'id_tbcmsproduct_comment'}
                            <div class="pull-right">
                                <a onclick="$(this).closest('tr').next('tr').show();" class="btn btn-small btn-default"><i class="icon-pencil"></i> {l s='Reply' mod='tbcmsproductcomments'}</a>
                            </div>
                        {/if}
                    {/if}
                </td>
            ...
            {if Module::getInstanceByName('reviewsreply')->active == true && $identifier == 'id_tbcmsproduct_comment'}
                <tr style="display:none;">
                    <td colspan="8" class="center" style="padding:8px">
                        {assign var='reply' value=rreply::getByIdTbcmsProductCommentAll($tr.id_tbcmsproduct_comment)}
                        <form method="post">
                            <input type="hidden" name="action" value="addreply"/>
                            <input type="hidden" name="id_tbcmsproduct_comment" value="{$tr.id_tbcmsproduct_comment}"/>
                            {if isset($reply.0)}
                            ...
                            <div class="col-lg-12">
                                <button class="btn btn-large btn-success">
                                    <i class="icon-save"></i> {if isset($reply.0)}{l s='Save reply' mod='tbcmsproductcomments'}{else}{l s='Add reply' mod='tbcmsproductcomments'}{/if}
                                </button>
                            </div>
                        </form>
                    </td>
    ...
    <tr>
        <td class="list-empty" colspan="{count($fields_display)+1}">
            <div class="list-empty-msg">
                ...
                {l s='No records found' mod='tbcmsproductcomments'}
            </div>
        </td>
    </tr>
{/if}
