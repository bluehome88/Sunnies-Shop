...
class ArSeoProURLProduct extends ArSeoProURLAbstract
{
    ...
    public function dispatch()
    {
        ...
        if (Tools::getValue('ars_rewrite_product')) {
            ...
            if (!$id_product) {
                if ($this->redirect == self::REDIRECT_CAT) {
                    $id_category = $this->owner->category->getLastIdFromCategoriesRewrite(
                    ...
                    if ($id_category != $context->shop->id_category) {
                        ...
                        $this->owner->redirect($redirect_url, ArSeoHelpers::getResponseHeader(301));
                    }

                    $this->owner->redirect($context->link->getPageLink('index'), ArSeoHelpers::getResponseHeader(301));
                } else {
    ...
    public function redirectSelectOptions()
    {
        return array(
            array(
                ...
                'name' => $this->l('None', 'ArSeoProURLProduct')
            ),
            array(
                'id' => self::REDIRECT_CAT,
                'name' => $this->l('Redirect to category', 'ArSeoProURLProduct')
            ),
            array(
                'id' => self::REDIRECT_404,
                'name' => $this->l('Redirect to page not found', 'ArSeoProURLProduct')
            )
        );
    }
