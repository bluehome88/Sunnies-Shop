...
class TbcmsPostsClass extends ObjectModel
{
    public $id;
    public $id_tbcmsposts;
    public $post_author;
    public $post_date;
    ...
    {
        Shop::addTableAssociation('tbcmsposts', array('type' => 'shop'));
    ...
    public function update($null_values = false)
    {
        if (isset($_FILES['post_img'])
            ...
            $tmp = Tools::getValue('id_tbcmsposts');
            $res = $this->getSinglePost($tmp);
            $categories = TbcmsImageTypeClass::getAllImageTypes();
            if (file_exists(TBCMSBLOG_IMG_DIR.$res['post_img'])) {
                unlink(TBCMSBLOG_IMG_DIR.$res['post_img']);
            }

            foreach ($categories as $category) {
                if (file_exists(TBCMSBLOG_IMG_DIR.$category['name'].'-'.$res['post_img'])) {
                    unlink(TBCMSBLOG_IMG_DIR.$category['name'].'-'.$res['post_img']);
                }
            }

            $this->post_img = TbcmsBlog::uploadMedia('post_img');
        } else {
            $id_tbcmsposts = Tools::getValue('id_tbcmsposts');
            $img = $this->getSinglePost($id_tbcmsposts);
            $this->post_img = $img['post_img'];
        }
        $tbaudio_temp = Tools::getValue('audio_temp');
        if ($tbaudio_temp
            && !empty($tbaudio_temp)
            && is_array($tbaudio_temp)) {
            $this->audio = @implode(",", $tbaudio_temp);
        }
        $tbvideo_temp = Tools::getValue('video_temp');
        if ($tbvideo_temp
            && !empty($tbvideo_temp)
            && is_array($tbvideo_temp)) {
            $this->video = @implode(",", $tbvideo_temp);
        }
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $gallery_temp = TbcmsBlog::bulkuploadMedia("gallery_temp");
    ...
    public function galleryUploadsUpdated()
    {
        ...
        $main_update_gallery = '';
        $gallery_temp = TbcmsBlog::bulkuploadMedia("gallery_temp");
        $tbgallery_temp_delete = Tools::getValue("gallery_temp_delete");
        if ($tbgallery_temp_delete && !empty($tbgallery_temp_delete)) {
            $gallery_temp_delete = @explode(",", $tbgallery_temp_delete);
        }
        ...
        if (isset($gallery_temp_delete)) {
            foreach ($gallery_temp_delete as $img) {
                if (file_exists(TBCMSBLOG_IMG_DIR.$img)) {
                    unlink(TBCMSBLOG_IMG_DIR.$img);
                }
                foreach ($categories as $category) {
                    if (file_exists(TBCMSBLOG_IMG_DIR.$category['name'].'-'.$img)) {
                        unlink(TBCMSBLOG_IMG_DIR.$category['name'].'-'.$img);
                    }
                }
            }
        }
    ...
    public static function getsinglepath($id_post = null, $post_type = 'post')
    {
        ...
        $params = array();
        $params['id'] = $categories['id_tbcmscategory'] ? $categories['id_tbcmscategory'] : 0;
        ...
        $link = TbcmsBlog::tbcmsBlogCategoryLink($params);
        $meta_title = Configuration::get(TbcmsBlog::$tbcmsblogshortname."meta_title");
        $meta_title = (isset($meta_title) ? $meta_title : "Blog");
        $blog_url = TbcmsBlog::tbcmsBlogLink();
    ...
    public function updatePosition($way, $position)
    {
        ...
        if (!empty($res)) {
            foreach ($res as $tbcmsposts) {
                if ((int)$tbcmsposts['id_tbcmsposts'] == (int)$this->id) {
                    $moved_tbcmsposts = $tbcmsposts;
                }
            }
        }
        if (!isset($moved_tbcmsposts) || !isset($position)) {
            return false;
        }
        $queryx = ' UPDATE `'._DB_PREFIX_.'tbcmsposts`
        SET `position`= `position` '.($way ? '- 1' : '+ 1').'
        WHERE `position`
        '.($way
        ? '> '.(int)$moved_tbcmsposts['position'].' AND `position` <= '.(int)$position
        : '< '.(int)$moved_tbcmsposts['position'].' AND `position` >= '.(int)$position.'
        ');
        $queryy = ' UPDATE `'._DB_PREFIX_.'tbcmsposts`
        SET `position` = '.(int)$position.'
        WHERE `id_tbcmsposts` = '.(int)$moved_tbcmsposts['id_tbcmsposts'];
        ...
        if ($id_post == null || $category_ids == null) {
            ...
            if (isset($category_ids) && !empty($category_ids)) {
                ...
                if (Db::getInstance()->execute('INSERT INTO `'._DB_PREFIX_
                        .'tbcms_category_post`(`id_post`, `id_category`,`type`) VALUES '.$queryval)) {
                    return true;
                } else {
                    return false;
                }
    ...
    public static function deleteTagPost($id_post = null, $tag = 'tag')
    {
        ...
        if (Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'tbcms_category_post WHERE id_post = '
    ...
    public static function getTheTitle($id_post = null, $post_type = 'post')
    {
        ...
        $sql = 'SELECT xc.`id_tbcmsposts`,xc.`category`,xcl.`post_title`,xcl.`link_rewrite` FROM `'
            ._DB_PREFIX_.'tbcmsposts` xc 
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_lang` xcl ON (xc.`id_tbcmsposts` = xcl.`id_tbcmsposts` '
            .'AND xcl.`id_lang` = '.$id_lang.')
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON (xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
            .'AND xcs.`id_shop` = '.$id_shop.')
        ';
        $sql .= ' WHERE xc.`post_type` = "'.($post_type?$post_type:'post').'" AND xc.`id_tbcmsposts` = '.$id_post;
    ...
    public static function getTheId($rewrite = null, $post_type = 'post')
    {
        ...
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xc.`id_tbcmsposts` FROM `'._DB_PREFIX_.'tbcmsposts` xc INNER JOIN `'._DB_PREFIX_
            .'tbcmsposts_lang` xcl ON (xc.`id_tbcmsposts` = xcl.`id_tbcmsposts` AND xcl.`id_lang` = '
            .$id_lang.') INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON '
            .'(xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
            .'AND xcs.`id_shop` = '.$id_shop.') ';
        $sql .= ' WHERE xc.`post_type` = "'.($post_type ? $post_type : 'post').'" AND xcl.`link_rewrite` = "'
            .$rewrite.'" ';
        $rslts = Db::getInstance()->getrow($sql);
        return isset($rslts['id_tbcmsposts']) ? $rslts['id_tbcmsposts'] : null;
    }
    ...
    public static function postExists($id_post = null, $post_type = 'post')
    {
        ...
        $sql = 'SELECT xc.`id_tbcmsposts` FROM `'._DB_PREFIX_.'tbcmsposts` xc WHERE xc.`post_type` = "'
            .($post_type ? $post_type : 'post').'" AND xc.active = 1 AND xc.`id_tbcmsposts` = '.$id_post;
        $rslts = Db::getInstance()->getrow($sql);
        
        $tmp = $rslts['id_tbcmsposts'];
    ...
    public static function getTheRewrite($id = null, $post_type = 'post')
    {
        ...
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xcl.`link_rewrite` FROM `'._DB_PREFIX_.'tbcmsposts` xc INNER JOIN `'._DB_PREFIX_
            .'tbcmsposts_lang` xcl ON (xc.`id_tbcmsposts` = xcl.`id_tbcmsposts` AND xcl.`id_lang` = '
            .$id_lang.') INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON '
            .'(xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
            .'AND xcs.`id_shop` = '.$id_shop.') ';
        $sql .= ' WHERE xc.`post_type` = "'.($post_type ? $post_type : 'post')
            .'" AND xc.`id_tbcmsposts` = "'.$id.'" ';
        $rslts = Db::getInstance()->getrow($sql);
    ...
    public static function getTheCategory($id_category = null, $category_type = 'category')
    {
        ...
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xc.`id_tbcmscategory`,xcl.`name`,xcl.`link_rewrite` FROM `'._DB_PREFIX_.'tbcmscategory` xc 
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory_lang` xcl ON (xc.`id_tbcmscategory` = xcl.`id_tbcmscategory` '
            .'AND xcl.`id_lang` = '.$id_lang.')
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory_shop` xcs ON (xc.`id_tbcmscategory` = xcs.`id_tbcmscategory` '
            .'AND xcs.`id_shop` = '.$id_shop.')
        ';
        $sql .= ' WHERE xc.`category_type` = "'.($category_type ? $category_type : 'category').'" AND'
            .' xc.`id_tbcmscategory` = '.$id_category;
        $rslts = Db::getInstance()->getrow($sql);
        return $rslts;
    }
    ...
    public static function tagInsert($tag = null)
    {
        ...
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xc.`id_tbcmscategory` FROM `'._DB_PREFIX_.'tbcmscategory` xc 
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory_lang` xcl ON (xc.`id_tbcmscategory` = xcl.`id_tbcmscategory` '
            .'AND xcl.`id_lang` = '.$id_lang.')
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory_shop` xcs ON (xc.`id_tbcmscategory` = xcs.`id_tbcmscategory` '
            .'AND xcs.`id_shop` = '.$id_shop.')
        ...
        if (isset($rslts) && !empty($rslts)) {
            return $rslts['id_tbcmscategory'];
        } else {
            $tagobj = new TbcmsCategoryClass();
    ...
    public static function getPostTags($id_post = null, $tag = 'tag')
    {
        ...
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xcl.`name` FROM `'._DB_PREFIX_.'tbcms_category_post` xcp 
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory` xc ON (xcp.`id_category` = xc.`id_tbcmscategory` '
            .'AND xc.`category_type` = "'.$tag.'")
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory_lang` xcl ON (xcp.`id_category` = xcl.`id_tbcmscategory` '
            .'AND xcl.`id_lang` = '.$id_lang.')
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory_shop` xcs ON (xcp.`id_category` = xcs.`id_tbcmscategory` '
            .'AND xcs.`id_shop` = '.$id_shop.')
        ';
    ...
    public static function getPostTagsResults($id_post = null, $tag = 'tag')
    {
        ...
        $id_lang = (int)Context::getContext()->language->id; // it use in id_lang from id_shop
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xcp.`id_category`,xcl.`name`,xcl.`link_rewrite` FROM `'._DB_PREFIX_.'tbcms_category_post` xcp 
        INNER JOIN `'._DB_PREFIX_.'tbcmscategory` xc ON (xcp.`id_category` = xc.`id_tbcmscategory` AND '
        ...
        if (isset($rslts) && !empty($rslts)) {
            ...
            foreach ($rslts as $rslt) {
                ...
                if ($tag == 'tag') {
                    ...
                    $results[$i]['link'] = TbcmsBlog::tbcmsBlogTagLink(array(
                        'id'=>$rslt['id_category'],
                    ...
                    $results[$i]['link'] = TbcmsBlog::tbcmsBlogCategoryLink(array(
                        'id'=>$rslt['id_category'],
                        'rewrite'=>$rslt['link_rewrite'],
                        'page_type'=>'category',
                        'subpage_type'=>'post'
    ...
    public static function getBlogTags($count = 10, $tag = 'tag')
    {
        ...
        $sql .= ' ORDER BY xc.`id_tbcmscategory` DESC ';
        $sql .= ' LIMIT '.(int)$count;
        $rslts = Db::getInstance()->executeS($sql);
        if (isset($rslts) && !empty($rslts)) {
            $i = 0;
            foreach ($rslts as $rslt) {
                $results[$i]['name'] = $rslt['name'];
                if ($tag == 'tag') {
                    $results[$i]['id_tag'] = $rslt['id_tbcmscategory'];
                    $results[$i]['link'] = TbcmsBlog::tbcmsBlogTagLink(array(
                        'id'=>$rslt['id_tbcmscategory'],
                        'rewrite'=>$rslt['link_rewrite'],
                        'page_type'=>'tag',
                        'subpage_type'=>'post'
                    ));
                } elseif ($tag == 'category') {
                    $results[$i]['id_category'] = $rslt['id_tbcmscategory'];
                    $results[$i]['link'] = TbcmsBlog::tbcmsBlogCategoryLink(array(
                        'id'=>$rslt['id_tbcmscategory'],
                        'rewrite'=>$rslt['link_rewrite'],
                        'page_type'=>'category',
    ...
    public static function getCategoryPostsCount($category = null, $post_type = 'post')
    {
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT count(xc.`id_tbcmsposts`) as alltbcmsposts FROM `'._DB_PREFIX_.'tbcmsposts` xc 
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_lang` xcl ON (xc.`id_tbcmsposts` = xcl.`id_tbcmsposts` '
            .'AND xcl.`id_lang` = '.$id_lang.')
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON (xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
            .'AND xcs.`id_shop` = '.$id_shop.')
        ...
        return (int)$queryexec['alltbcmsposts'];
    }
    ...
    public static function getCategoryPosts(
        ...
        if ($n == null) {
            $n = (int)Configuration::get(TbcmsBlog::$tbcmsblogshortname."post_per_page");
        }
        $results = array();
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $GetAllImageTypes = TbcmsImageTypeClass::getAllImageTypes();
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'tbcmsposts` xc 
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_lang` xcl ON (xc.`id_tbcmsposts` = xcl.`id_tbcmsposts` '
            .'AND xcl.`id_lang` = '.$id_lang.')
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON (xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
            .'AND xcs.`id_shop` = '.$id_shop.')
            ...
            foreach ($queryexec as $qlvalue) {
                if (isset($qlvalue) && !empty($qlvalue)) {
                    foreach ($qlvalue as $qkey => $qvalue) {
                        ...
                        if ($qkey == 'post_img') {
                            if (isset($GetAllImageTypes) && !empty($GetAllImageTypes)) {
                                foreach ($GetAllImageTypes as $imagetype) {
                                    $results[$i]['post_img_'.$imagetype['name']] = TBCMSBLOG_IMG_URI
                        ...
                        $results[$i]['link'] = TbcmsBlog::tbcmsBlogPostLink(array(
                            'id'=>$qlvalue['id_tbcmsposts'],
                            'rewrite'=>$qlvalue['link_rewrite'],
                            'page_type'=>$post_type
                        ));
                        $results[$i]['post_tags'] = self::getPostTagsResults($qlvalue['id_tbcmsposts'], "tag");
                        ...
                        if (isset($qlvalue['gallery']) && !empty($qlvalue['gallery'])) {
                            ...
                            if (isset($gallery_lists) && !empty($gallery_lists)) {
                                ...
                                foreach ($gallery_lists as $gall) {
                                    $results[$i]['gallery_lists'][$ij]['main'] = TBCMSBLOG_IMG_URI.$gall;
                                    if (isset($GetAllImageTypes) && !empty($GetAllImageTypes)) {
                                        foreach ($GetAllImageTypes as $imagetype) {
                                            $results[$i]['gallery_lists'][$ij][$imagetype['name']] = TBCMSBLOG_IMG_URI
                                                .$imagetype['name'].'-'.$gall;
                                            if (!self::imageExists($imagetype['name'].'-'.$gall)) {
                                                $tmp = TBCMSBLOG_IMG_URI.'noimage.jpg';
                        ...
                        if ($qkey == 'category') {
                            $category_arr = new TbcmsCategoryClass((int)$qvalue);
                            ...
                            $results[$i]['category_arr']['link'] = TbcmsBlog::tbcmsBlogCategoryLink(array(
                                'id'=>$category_arr->id,
                        ...
                        $post_id = $qlvalue['id_tbcmsposts'];
                        if (isset($post_id) && !empty($post_id)) {
                            $tbcmsblog_total_commets = TbcmsCommentClass::getCountComments($post_id);
                            $results[$i]['total_comments'] = $tbcmsblog_total_commets;
                        }
                    }
                    $i++;
                }
    ...
    public static function getRecentPosts($count = 4, $post_type = 'post', $order_by = 'DESC')
    {
        ...
        $sql .= ' ORDER BY xc.`id_tbcmsposts` '.$order_by;
        $sql .= ' LIMIT '.(int)$count;
        $queryexec = Db::getInstance()->executeS($sql);
        if (isset($queryexec) && !empty($queryexec)) {
            $i = 0;
    ...
    public static function postCountUpdate($id = null)
    {
        ...
        $sql = 'UPDATE '._DB_PREFIX_.'tbcmsposts as xc SET xc.comment_count = (xc.comment_count+1) '
            .'where xc.id_tbcmsposts = '.$id;
    ...
    public static function getSinglePost($id_post = null, $post_type = 'post')
    {
        ...
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'tbcmsposts` xc 
               ...
               INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON (xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
                    .'AND xcs.`id_shop` = '.$id_shop.')
               ';
        $sql .= ' WHERE xc.`active` = 1 AND xc.post_type = "'.$post_type.'" AND xc.id_tbcmsposts = '.(int)$id_post;
        $queryexec = Db::getInstance()->getrow($sql);
        if (isset($queryexec) && !empty($queryexec)) {
            foreach ($queryexec as $qkey => $qvalue) {
                ...
                if ($qkey == 'post_img') {
                    if (isset($GetAllImageTypes) && !empty($GetAllImageTypes)) {
                        foreach ($GetAllImageTypes as $imagetype) {
                            $tmp = TBCMSBLOG_IMG_URI.$imagetype['name'].'-'.$qvalue;
                            $results['post_img_'.$imagetype['name']] = $tmp;
                            if (!self::imageExists($imagetype['name'].'-'.$qvalue)) {
                                $results['post_img_'.$imagetype['name']] =  TBCMSBLOG_IMG_URI
                                    .'noimage.jpg';
                            }
                        }
                        if (!self::imageExists($qvalue)) {
                ...
                if ($qkey == 'gallery') {
                    ...
                    if (isset($gallery_lists) && !empty($gallery_lists)) {
                        $ij = 0;
                        foreach ($gallery_lists as $gall) {
                            $results['gallery_lists'][$ij]['main'] = TBCMSBLOG_IMG_URI.$gall;
                            if (isset($GetAllImageTypes) && !empty($GetAllImageTypes)) {
                                foreach ($GetAllImageTypes as $imagetype) {
                                    $results['gallery_lists'][$ij][$imagetype['name']] = TBCMSBLOG_IMG_URI
                                        .$imagetype['name'].'-'.$gall;
                                    if (!self::imageExists($imagetype['name'].'-'.$gall)) {
                                        $tmp = TBCMSBLOG_IMG_URI.'noimage.jpg';
                ...
                $results['link'] = TbcmsBlog::tbcmsBlogPostLink(array(
                    'id'=>$queryexec['id_tbcmsposts'],
                    'rewrite'=>$queryexec['link_rewrite'],
                    'page_type'=>$post_type
                ));
                $results['post_tags'] = self::getPostTagsResults($queryexec['id_tbcmsposts'], "tag");
                if ($qkey == 'category') {
                    $category_arr = new TbcmsCategoryClass((int)$qvalue);
                    ...
                    $results['category_arr']['link'] = TbcmsBlog::tbcmsBlogCategoryLink(array(
                        'id'=>$category_arr->id,
                        'rewrite'=>$category_arr->link_rewrite[$id_lang],
                        'subpage_type'=>$post_type,
                        'page_type'=>'category'
    ...
    public static function getTagPosts($id_tag = null, $p = null, $n = null, $post_type = 'post', $order_by = 'DESC')
    {
        ...
        INNER JOIN `'._DB_PREFIX_.'tbcmsposts_shop` xcs ON (xc.`id_tbcmsposts` = xcs.`id_tbcmsposts` '
            .'AND xcs.`id_shop` = '.$id_shop.')
        INNER JOIN `'._DB_PREFIX_.'tbcms_category_post` xcp ON (xcp.`id_post` = xc.`id_tbcmsposts` '
