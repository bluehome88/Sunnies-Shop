...
class TbcmsCategoryClass extends ObjectModel
{
    public $id;
    public $id_tbcmscategory;
    public $name;
    public $description;
    public $link_rewrite;
    public $category_img;
    ...
    {
        Shop::addTableAssociation('tbcmscategory', array('type' => 'shop'));
    ...
    public static function categoryExists($id_category = null, $category_type = 'post')
    {
        ...
        $sql = 'SELECT xc.`id_tbcmscategory` FROM `'._DB_PREFIX_.'tbcmscategory` xc WHERE '
            .'xc.`category_type` = "'.($category_type ? $category_type : 'category')
            .'" AND xc.active = 1 AND xc.`id_tbcmscategory` = '.$id_category;
        $rslts = Db::getInstance()->getrow($sql);
            return (isset($rslts['id_tbcmscategory']) && !empty($rslts['id_tbcmscategory'])) ? true : false;
    }
    ...
    public function update($null_values = false)
    {
        if (isset($_FILES['category_img'])
            ...
            if (file_exists(TBCMSBLOG_IMG_DIR.$res['category_img'])) {
                unlink(TBCMSBLOG_IMG_DIR.$res['category_img']);
            }
            $this->category_img = TbcmsBlog::uploadMedia('category_img');
        } else {
            $sql = 'SELECT * FROM `'._DB_PREFIX_.'tbcmscategory` WHERE '
            .' `id_tbcmscategory` = '.Tools::getValue('id_tbcmscategory');
    ...
    public static function getTopPosition()
    {
        $sql = 'SELECT MAX(`position`)
                FROM `'._DB_PREFIX_.'tbcmscategory`';
    ...
    public function updatePosition($way, $position)
    {
        if (!$res = Db::getInstance()->executeS(
            'SELECT `id_tbcmscategory`, `position`
            FROM `'._DB_PREFIX_.'tbcmscategory`
        ...
        if (!empty($res)) {
            foreach ($res as $tbcmscategory) {
                if ((int)$tbcmscategory['id_tbcmscategory'] == (int)$this->id) {
                    $moved_tbcmscategory = $tbcmscategory;
                }
            }
        }
        if (!isset($moved_tbcmscategory) || !isset($position)) {
            return false;
        }
        $queryx = ' UPDATE `'._DB_PREFIX_.'tbcmscategory`
        SET `position`= `position` '.($way ? '- 1' : '+ 1').'
        WHERE `position`
        '.($way
        ? '> '.(int)$moved_tbcmscategory['position'].' AND `position` <= '.(int)$position
        : '< '.(int)$moved_tbcmscategory['position'].' AND `position` >= '.(int)$position.'
        ');
        $queryy = ' UPDATE `'._DB_PREFIX_.'tbcmscategory`
        SET `position` = '.(int)$position.'
        WHERE `id_tbcmscategory` = '.(int)$moved_tbcmscategory['id_tbcmscategory'];
    ...
    public static function getTheTitle($id_category = null, $category_type = 'category')
    {
        ...
        $sql = 'SELECT xc.`id_tbcmscategory`,xcl.`name`,xcl.`link_rewrite` FROM `'._DB_PREFIX_
            .'tbcmscategory` xc INNER JOIN `'._DB_PREFIX_.'tbcmscategory_lang` xcl ON (xc.`id_tbcmscategory` = '
            .'xcl.`id_tbcmscategory` AND xcl.`id_lang` = '.$id_lang.') INNER JOIN `'._DB_PREFIX_
            .'tbcmscategory_shop` xcs ON (xc.`id_tbcmscategory` = xcs.`id_tbcmscategory` AND xcs.`id_shop` = '
            .$id_shop.') ';
    ...
    public static function getTheId($rewrite = null, $category_type = 'category')
    {
        ...
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT xc.`id_tbcmscategory` FROM `'._DB_PREFIX_.'tbcmscategory` xc INNER JOIN `'._DB_PREFIX_
            .'tbcmscategory_lang` xcl ON (xc.`id_tbcmscategory` = xcl.`id_tbcmscategory` AND xcl.`id_lang` = '
            .$id_lang.') INNER JOIN `'._DB_PREFIX_.'tbcmscategory_shop` xcs ON (xc.`id_tbcmscategory` = '
            .'xcs.`id_tbcmscategory` AND xcs.`id_shop` = '.$id_shop.') ';
        $sql .= ' WHERE xc.`category_type` = "'.($category_type ? $category_type : 'category')
            .'" AND xcl.`link_rewrite` = "'.$rewrite.'" ';
        $rslts = Db::getInstance()->getrow($sql);
        return isset($rslts['id_tbcmscategory']) ? $rslts['id_tbcmscategory'] : null;
    }
    ...
    public static function getCategories($category_type = 'category', $category_group = null)
    {
        $id_lang = (int)Context::getContext()->language->id;
        $id_shop = (int)Context::getContext()->shop->id;
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'tbcmscategory` xc 
               INNER JOIN `'._DB_PREFIX_.'tbcmscategory_lang` xcl ON
                    (xc.`id_tbcmscategory` = xcl.`id_tbcmscategory` '
                    .'AND xcl.`id_lang` = '.$id_lang.')
               INNER JOIN `'._DB_PREFIX_.'tbcmscategory_shop` xcs ON
                    (xc.`id_tbcmscategory` = xcs.`id_tbcmscategory` '
    ...
    public static function serializeCategory($brief = true)
    {
        ...
        if (isset($category) && !empty($category)) {
            ...
            foreach ($category as $categoryvalue) {
                $results[$i]['id'] = $categoryvalue['id_tbcmscategory'];
