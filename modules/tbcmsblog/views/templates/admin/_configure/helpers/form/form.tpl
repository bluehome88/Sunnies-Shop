								...
								{elseif $input.type == 'switch'}
									<span class="switch prestashop-switch fixed-width-lg">
										...
										<label {if $value.value == 1} for="{$input.name}_on"{else} for="{$input.name}_off"{/if}>
											{if $value.value == 1}
												{l s='Yes' mod='tbcmsblog'}
											{else}
												{l s='No' mod='tbcmsblog'}
											{/if}
										</label>
										{/strip}
										{/foreach}
								...
								{elseif $input.type == 'change-password'}
									<div class="row">
										<div class="col-lg-12">
											<button type="button" id="{$input.name}-btn-change" class="btn btn-default">
												<i class='material-icons'>&#xe897;</i>
												{l s='Change password...' mod='tbcmsblog'}
											</button>
											<div id="{$input.name}-change-container" class="form-password-change well hide">
												<div class="form-group">
													<label for="old_passwd" class="control-label col-lg-2 required">
														{l s='Current password' mod='tbcmsblog'}
													</label>
													<div class="col-lg-10">
												...
												<div class="form-group">
													<label for="{$input.name}" class="required control-label col-lg-2">
														<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="Password should be at least 8 characters long.">						
															{l s='New password' mod='tbcmsblog'}
														</span>
													</label>
													<div class="col-lg-9">
														<div class="input-group fixed-width-lg">
												...
												<div class="form-group">
													<label for="{$input.name}2" class="required control-label col-lg-2">
														{l s='Confirm password' mod='tbcmsblog'}
													</label>
													<div class="col-lg-4">
														<div class="input-group fixed-width-lg">
															<span class="input-group-addon">
												...
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														...
														<button type="button" id="{$input.name}-generate-btn" class="btn btn-default">
															...
															{l s='Generate password' mod='tbcmsblog'}
														</button>
													</div>
												</div>
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														<p class="checkbox">
															<label for="{$input.name}-checkbox-mail">
																...
																{l s='Send me this new password by Email' mod='tbcmsblog'}
															</label>
														</p>
													</div>
												</div>
									...
									<script>
										$(function(){
											...
											var feedback = [
												{ badge: 'text-danger', text: '{l s='Invalid' mod='tbcmsblog' js=1}' },
												{ badge: 'text-warning', text: '{l s='Okay' mod='tbcmsblog' js=1}' },
												{ badge: 'text-success', text: '{l s='Good' mod='tbcmsblog' js=1}' },
												{ badge: 'text-success', text: '{l s='Fabulous' mod='tbcmsblog' js=1}' }
											];
											...
											}, '{l s='Invalid password confirmation' mod='tbcmsblog' js=1}');
										...
										<select name="{$key}" class="fixed-width-lg{if isset($input.class)} {$input.class}{/if}">
											...
											{if $key == 'months'}
												{*
													...
													{l s='January' mod='tbcmsblog'}
													{l s='February' mod='tbcmsblog'}
													{l s='March' mod='tbcmsblog'}
													{l s='April' mod='tbcmsblog'}
													{l s='May' mod='tbcmsblog'}
													{l s='June' mod='tbcmsblog'}
													{l s='July' mod='tbcmsblog'}
													{l s='August' mod='tbcmsblog'}
													{l s='September' mod='tbcmsblog'}
													{l s='October' mod='tbcmsblog'}
													{l s='November' mod='tbcmsblog'}
													{l s='December' mod='tbcmsblog'}
												*}
												{foreach $select as $k => $v}
													<option value="{$k}" {if $k == $fields_value[$key]}selected="selected"{/if}>{l s=$v mod='tbcmsblog'}</option>
												{/foreach}
											{else}
												{foreach $select as $v}
				...
				{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
					<div class="panel-footer">
						...
						<a href="{$back_url|escape:'html':'UTF-8'}" class="btn btn-default" onclick="window.history.back();">
							<i class="process-icon-cancel"></i> {l s='Cancel' mod='tbcmsblog'}
						</a>
						{/if}
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('.datetimepicker').datetimepicker({
				...
				currentText: '{l s='Now' mod='tbcmsblog'}',
				closeText: '{l s='Done' mod='tbcmsblog'}',
				ampm: false,
				amNames: ['AM', 'A'],
				pmNames: ['PM', 'P'],
				timeFormat: 'hh:mm:ss tt',
				timeSuffix: '',
				timeOnlyTitle: '{l s='Choose Time' mod='tbcmsblog' js=1}',
				timeText: '{l s='Time' mod='tbcmsblog' js=1}',
				hourText: '{l s='Hour' mod='tbcmsblog' js=1}',
				minuteText: '{l s='Minute' mod='tbcmsblog' js=1}',
			});
...
<div class="panel" id="fieldset_generate_img">
	<div class="panel-heading">
		<i class="icon-AdminParentPreferences"></i>  {l s='Regenerate Image Thumbnail' mod='tbcmsblog'}
	</div>
	<div class="form-wrapper">
		<div class="form-group">
			<label class="control-label col-lg-3">
				{l s='Select Image Type' mod='tbcmsblog'}
			</label>
			<div class="col-lg-9">
				<select name="select_image_type" class=" fixed-width-xl" id="select_image_type">
					<option value="all">{l s='ALL Blog Image' mod='tbcmsblog'}</option>
					<option value="post">{l s='All Blog Post Image' mod='tbcmsblog'}</option>
					<option value="gallery">{l s='All Blog Post Gallery Image' mod='tbcmsblog'}</option>
					<option value="category">{l s='All Blog Category Image' mod='tbcmsblog'}</option>
				</select>
				<p class="help-block">
					{l s='Please Select Which Type of Image You Want To Regenerate.' mod='tbcmsblog'}
				</p>
			</div>
		</div>
	</div>
	<div class="panel-footer">
		<button type="submit" value="1" id="submit_generateimage" name="submit_generateimage" class="btn btn-default pull-right">
			<i class="process-icon-save"></i> {l s='Save' mod='tbcmsblog'}
		</button>
