...
class AdmintbcmspostController extends ModuleAdminController
{
    public function __construct()
    {
        $this->table = 'tbcmsposts';
        $this->className = 'TbcmsPostsClass';
        $this->lang = true;
        $this->deleted = false;
    ...
    public function init()
    {
        ...
        if (Tools::getIsset('deletettbcmsposts') && !empty($tmp)) {
            $res = TbcmsPostsClass::getSinglePost($tmp);
            ...
            foreach ($img as $img_type) {
                if (file_exists(TBCMSBLOG_IMG_DIR.$img_type)) {
                    unlink(TBCMSBLOG_IMG_DIR.$img_type);
                }
                foreach ($categories as $category) {
                    if (file_exists(TBCMSBLOG_IMG_DIR.$category['name'].'-'.$img_type)) {
                        unlink(TBCMSBLOG_IMG_DIR.$category['name'].'-'.$img_type);
                    }
                }
            }
        }
        ...
        parent::init();
        $this->_join = 'LEFT JOIN '._DB_PREFIX_.'tbcmsposts_shop sbp ON a.id_tbcmsposts=sbp.id_tbcmsposts '
        ...
        if (Shop::isFeatureActive() && Shop::getContext() != Shop::CONTEXT_SHOP) {
            $this->_group = 'GROUP BY a.id_tbcmsposts';
        }
    ...
    public function renderForm()
    {
        ...
        $gallery_temp_str = '';
        $post_img_temp = '';
        if (isset($id_tbcmsposts) && !empty($id_tbcmsposts)) {
            $tbcmspostsclass = new TbcmsPostsClass($id_tbcmsposts);
            if (isset($tbcmspostsclass->audio) && !empty($tbcmspostsclass->audio)) {
                $audio_temp = @explode(",", $tbcmspostsclass->audio);
            }
            if (isset($tbcmspostsclass->video) && !empty($tbcmspostsclass->video)) {
                $video_temp = @explode(",", $tbcmspostsclass->video);
            }
            if (isset($tbcmspostsclass->gallery) && !empty($tbcmspostsclass->gallery)) {
                $gallery_temp = @explode(",", $tbcmspostsclass->gallery);
                $gallery_temp_str = $tbcmspostsclass->gallery;
            }
            if (isset($tbcmspostsclass->post_img) && !empty($tbcmspostsclass->post_img)) {
                $post_img_temp = '<img src="'.TBCMSBLOG_IMG_URI.$tbcmspostsclass->post_img.'" height="110" '
                    .'width="auto"><br>';
            }
        }
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                //     'values' => array(
                //         'query' => TbcmsCategoryClass::serializeCategory(false),
                //         'id' => 'id',
                //         'name' => 'name'
                //     )
                // ),
        ...
        if (!($tbcmspostsclass = $this->loadObject(true))) {
            return;
        }
        $this->setdefaultbalue($tbcmspostsclass);
    ...
    public function setdefaultbalue($obj)
    {
        ...
        if (isset($obj->id) && !empty($obj->id)) {
            $this->fields_value['meta_tag'] = TbcmsPostsClass::getPostTags($obj->id);
        } else {
    ...
    public function processPosition()
    {
        ...
        } elseif (!Validate::isLoadedObject($object = new TbcmsPostsClass((int)Tools::getValue(
            $this->identifier,
            Tools::getValue('id_tbcmsposts', 1)
        )))) {
        ...
        if (!$object->updatePosition((int)Tools::getValue('way'), (int)Tools::getValue('position'))) {
            ...
            Tools::redirectAdmin(self::$currentIndex.'&'.$this->table.'Orderby=position&'.$this->table
                .'Orderway=asc&conf=5'.(($id_tbcmsposts = (int)Tools::getValue($this->identifier)) ? ('&'
                    .$this->identifier.'='.$id_tbcmsposts) : '').'&token='
    ...
    {
        $id_tbcmsposts = (int)(Tools::getValue('id'));
        $way = (int)(Tools::getValue('way'));
        $positions = Tools::getValue($this->table);
        if (is_array($positions)) {
            foreach ($positions as $key => $value) {
                $pos = explode('_', $value);
                if ((isset($pos[1]) && isset($pos[2])) && ($pos[2] == $id_tbcmsposts)) {
                    $position = $key + 1;
                    break;
                }
            }
        ...
        $tbcmspostsclass = new TbcmsPostsClass($id_tbcmsposts);
        if (Validate::isLoadedObject($tbcmspostsclass)) {
            if (isset($position) && $tbcmspostsclass->updatePosition($way, $position)) {
                Hook::exec('action'.$this->className.'Update');
                die(true);
            } else {
                die('{"hasError" : true, errors : "Can not update tbcmspostsclass position"}');
            }
        } else {
            die('{"hasError" : true, "errors" : "This tbcmspostsclass can not be loaded"}');
        }
    }
}
