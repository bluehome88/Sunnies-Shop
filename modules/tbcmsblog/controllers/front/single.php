...
class TbcmsBlogSingleModuleFrontController extends TbcmsBlogMainModuleFrontController
{
    public $blogpost;
    public $tbcmserrors;
    public $id_identity;
    ...
    public function init()
    {
        ...
        if (!isset($id_identity) || empty($id_identity)) {
            $this->id_identity = (int)TbcmsPostsClass::getTheTd($this->rewrite, $this->page_type);
        } else {
            $this->id_identity = (int)$id_identity;
        }
        if (!TbcmsPostsClass::PostExists($this->id_identity, $this->page_type)) {
        ...
        if (!$this->id_identity || !Validate::isUnsignedId($this->id_identity)) {
            ...
            $this->blogpost = TbcmsPostsClass::getSinglePost($this->id_identity);
            TbcmsPostsClass::postCountUpdate($this->id_identity);
        }
    }
    ...
    public function initContent()
    {
        ...
        if (isset($this->id_identity) && !empty($this->id_identity)) {
            $tbcmsblog_commets = TbcmsCommentClass::getComments($this->id_identity);
            $this->context->smarty->assign('tbcmsblog_commets', $tbcmsblog_commets);
        }
        if (isset($this->tbcmserrors) && !empty($this->tbcmserrors)) {
            $this->context->smarty->assign('tbcmserrors', $this->tbcmserrors);
        }
        $path = TbcmsPostsClass::getsinglepath($this->id_identity, $this->page_type);
        $this->context->smarty->assign('path', $path);

        $disable_blog_com = (int)Configuration::get(TbcmsBlog::$tbcmsblogshortname."disable_blog_com");

        $this->context->smarty->assign('disable_blog_com', $disable_blog_com);

        $this->context->smarty->assign('tbcmsblog_dir', _PS_MODULE_DIR_.TbcmsBlog::$ModuleName);
        $this->context->smarty->assign('tbcmsblog_uri', __PS_BASE_URI__.TbcmsBlog::$ModuleName);
        $this->context->smarty->assign('tbcmsblog_img_uri', __PS_BASE_URI__.TbcmsBlog::$ModuleName.'/img/');
        $this->context->smarty->assign('tbcmsblog_css_uri', __PS_BASE_URI__.TbcmsBlog::$ModuleName.'/css/');
        $this->context->smarty->assign('tbcmsblog_js_uri', __PS_BASE_URI__.TbcmsBlog::$ModuleName.'/js/');
        $template = "single.tpl";
    ...
    {
        $entity = 'module-tbcmsblog-single';
